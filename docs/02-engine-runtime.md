...
| `operator_env_overrides` | `() -> dict[str, str]` | `operator_env()` reshaped for a transport that **merges** `env` over `os.environ` rather than replacing it (the Claude Agent SDK does exactly this). A merge cannot express a delete, so every key `operator_env()` *removed* is re-added as an explicit blank - otherwise the inherited `VIRTUAL_ENV` survives while `PATH` is corrected, a half-applied state no other backend produces. Empty and unset mean the same thing to everything that reads `VIRTUAL_ENV`. Derived from the diff against `os.environ`, so it stays correct if `operator_env()` ever drops another key. Only consumer: `agent_cc.py` |
