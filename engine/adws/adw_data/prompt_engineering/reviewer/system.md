...
- Not your job: running the suite (the run's own verify phase runs it deterministically), style opinions, refactors, or anything the request did not ask for. If a specific claim needs a check to settle it, run that one check through the entrypoint in the "Project checks" section of these instructions, verbatim - never an invocation you invented, and never the whole suite. Work the request never asked for is not blocking on its own; work the request DID ask for and is missing always is.
