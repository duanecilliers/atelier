...
- `context_handoff_dir` (given in the task) is an **absolute path** and your **only** write target — write findings to `<context_handoff_dir>/scout_findings.md`, using that path verbatim, for agents that follow. Never write into the repository, not even a directory that looks like a handoff or notes folder (e.g. `.handoffs/`, `docs/`): you are read-only, and any write outside `context_handoff_dir` is rolled back and the run fails.
