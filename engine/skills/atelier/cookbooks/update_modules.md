...
| Module | Owns |
|---|---|
| `data_types.py` | Every Pydantic model: `AgentCall`, `PhaseParams`, `Phase`, `EnvelopeBase` + one output type per agent call, the config models (`AgentConfig`, `SSSFConfig`), `EventRecord`, and the shared backend request/result types `PiRequest`/`PiResult` (pi-named historically; `agent_cc` takes and returns them too — same `PiRequest` in, same `PiResult` out) |
| `agents.py` | `load_config`, `validate`, resolving an entry → coding-agent interface + model + thinking + harness extensions. Forces `coding_agent: claude_code` for any `anthropic/*` model — pi no longer supports Anthropic — overriding even an explicit `coding_agent: pi`, and injects the repo-root guidance (`AGENTS.md`, else `CLAUDE.md`) into a `claude_code` agent's system prompt |
| `runner.py` | the `Run` object; `run.phase(PhaseParams)` context manager; `ph.call(AgentCall)` |
| `agent_cc.py` | the Claude Code backend (the default) — drives Claude via the `claude-agent-sdk` on the local `claude` CLI's own login (no API key). Runs in isolation (`setting_sources: []`), so nothing ambient leaks in; mirrors `agent_pi.run`'s contract exactly, down to the environment it spawns under (`utils.operator_env_overrides()` - the merge-safe form of `operator_env()`, since the SDK merges `options.env` over `os.environ` instead of replacing it) |
| `agent_pi.py` | the pi backend — drives non-Anthropic models (e.g. `openai-codex/*`) via non-interactive `pi -p --mode json`, JSONL stream tailed live; an agent call re-prompts within its live session, so running and continuing an agent are the same call. (pi 0.81.1 ships no `~/.pi/agent/models.json`; `PI_MODELS_PATH` points at a committed stub, and `context_window()` degrades to 0/unknown when even that is absent) |
