...
| `agent_cc.py` | the Claude Code backend (the default) — drives Claude via the `claude-agent-sdk` on the local `claude` CLI's own login (no API key). Runs in isolation (`setting_sources: []`), so nothing ambient leaks in; mirrors `agent_pi.run`'s contract exactly |
