...
The system map the orchestrator reads on startup — what Atelier is, how a stamped repo is laid out, and which cookbook to load next.

## What Atelier is

Atelier builds repeatable **agents plus code** workflows. Deterministic Python (an ADW script) owns sequencing, retries, and acceptance; agents are bounded nodes inside that graph. Agents propose, deterministic code disposes.
...
│   ├── agent_cc.py              Claude Code backend (default)  ·  agent_pi.py  pi backend
...
└── adw_data/
    ...
    └── sssf.db                  gitignored SQLite trace db the cockpit reads
```

## Backends

An agent's `coding_agent:` picks its backend; both are real and `agents.execute` treats them identically.

- **`claude_code` is the default.** It drives Claude via the `claude-agent-sdk` using the local **`claude` CLI login — no API key.** The starter roster is `coding_agent: claude_code`, `model: anthropic/claude-sonnet-5`. Because the SDK runs in isolation (no ambient `CLAUDE.md`/skills leak in), the engine injects this repo's root guidance file — `AGENTS.md`, else `CLAUDE.md` — into the agent's system prompt so it sees the project's own conventions.
- **`pi`** drives **non-Anthropic** models via the `pi` CLI (auth in `~/.pi/agent`), e.g. `openai-codex/*`. It discovers `AGENTS.md`/`CLAUDE.md` from the cwd natively.
- **Anthropic is always routed through `claude_code`.** `agents.py::load_config` forces `coding_agent: claude_code` for any `anthropic/*` model, overriding even an explicit `coding_agent: pi` — pi no longer supports Anthropic, so a roster cannot mis-route it.
...
| Keep the engine current / how the stamp works | `install.md` |
