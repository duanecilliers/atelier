...
The event schema, the seven SQLite tables, and the polling contract — the one data path is **agents → sqlite → cockpit**.

## Two stores, one truth

**Files are the raw record** (`raw_output.jsonl` streams, `envelope.json`, `agent_map.json`); **SQLite (`sssf.db`) is the queryable mirror** the cockpit reads. `tracer.py` writes both. Losing the db loses nothing that can't be rebuilt from files.
...
| `tool_call` | a tool (e.g. `read`, `bash`, `edit`, `write`) returns — **one event per real call**, named `bash: ls -la src`, payload `{tool, tool_call_id, args, result_snippet, ok, duration_ms, agent}` |
...
`parent_id` nests spans, so an agent phase expands into its tool-call spans in the cockpit.

**Spend is itemised per phase.** `agent_end.usage` carries tokens *and* dollars for each component the backend reports — `input`, `output`, `cache_read`, `cache_write` — summed across every send the phase made, so a phase that retried on a bad envelope or a failed gate shows what all its attempts cost, not just the last one. The four components sum to `total_tokens`, and their costs sum to `total_cost`; the cockpit's Cost panel renders them as a table you can add up by eye.

`reasoning_tokens` is the thinking share and is **inside** `output_tokens`, not a fifth component — measured across every session on disk, reasoning never exceeds output and the four components always reconcile to the total. It bills at the output rate, so the panel nests it under output rather than adding it. Runs predating the breakdown have no `usage` key at all; the lump `cost` and the event's own `tokens` still stand, and the cockpit says so rather than rendering zeroes.

**Context is occupancy, not spend.** `events.tokens` and `sessions.total_tokens` bill every turn, so they only grow — an agent that burned 100k tokens may be sitting in a 15k window. `context_tokens` is how full the window actually was when the agent stopped, which is what the cockpit's per-lane Context bar measures against `context_window`.

It is computed the way a coding agent computes it for its own footer and its auto-compaction trigger: take the last *valid* assistant turn — skipping `aborted` and `error` turns — and read the reported total token usage, falling back to `input + output + cacheRead + cacheWrite`. Cache reads count; cached prompt is still prompt. `context_window` is the model's ceiling as the backend reports it, so `context_tokens / context_window` is the occupancy the agent itself would show. When a backend can't report a context window at all — e.g. a `pi` install with no models registry to read `contextWindow` from — `context_window` **degrades to 0/unknown** and the lane draws no bar rather than a misleading one. Both are also NULL on rows written before the columns existed, and again the lane draws no bar rather than a misleading empty one.

Two caveats worth knowing. A backend may add an *estimate* for any messages trailing the last assistant usage; in a single-shot batch run the session ends on that message, so the two agree. And if auto-compaction fires as the very last act of a run, the recorded number is the pre-compaction size — the backend itself reports `null` in that window rather than guessing.
...
**A `tool_call` is the one event that spans time**, so it fills both `started_at` and `ended_at` on the row — the tool's real start and return. Every other type is a point in time: `started_at` is when it was recorded and `ended_at` stays NULL. Lay tool calls out on a time axis from those columns, never by parsing `payload_json` (`duration_ms` is in the payload too, as the backend's own number, but it is a convenience, not the source for layout).

**Streaming is solved by construction.** The agent backend tails the coding agent's streamed output line by line and the tracer inserts each event into `sssf.db` **while the agent is still working** — never batched at phase end (verified in the first smoke run: tool calls visible mid-run). Everything downstream is a poll → render.
...
agent_sessions (                   -- the queryable mirror of agent_map.json
  ...
  context_window INTEGER,           -- the model's ceiling, as reported by the backend
  created_at    TEXT, last_used_at TEXT,
  PRIMARY KEY (adw_id, agent)
);
```

**A hung agent emits nothing**, which is exactly when you need its pid: no events, no tokens, no output to read. `processes` is the only table that can answer "what is this run running, and how do I stop it" — `just procs <adw_id>` lists what is live, and stopping a run goes through the cockpit's cancel (it sets `cancel_requested` and the worker SIGTERMs the process group) or a manual SIGTERM to the process group, verifying the recorded `command` still matches the pid before signalling it. A killed run finalizes its own trace: SIGTERM and SIGINT are turned into `SystemExit` in `session.ensure`, so the session lands on `fail` with its process rows closed instead of reading `running` forever.
...
Phase status invariants: `queued` only for manifest-declared phases not yet entered (dashed in the cockpit); `running` on enter; only a clean exit writes `success` — agent phases additionally need the envelope parsed and gates green; everything else resolves to `fail`.
...
WAL allows readers during writes. Writers are the tracers of running ADW processes; concurrent writers are fine given one small transaction per event plus `busy_timeout`. The cockpit reads on a readonly connection with exactly one exception: archiving a session opens a second connection to set `sessions.archived`. That flag is review triage — it says a human has looked at the run — so it is the reader's state living on the row, and no tracer ever writes or reads it.

## Polling contract

**The cockpit never receives pushes.** No ingest endpoint, no WebSocket, no backfill or dedup logic.
