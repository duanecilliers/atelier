...
| `branch` | The named branch the worktree checks out. Survives teardown in the shared `.git`. Precedence at create: an **explicit branch** entered on the form wins; else a **`purpose`** defers to `namer`; else this template (default `adw/${SANDBOX_ID}`). |
