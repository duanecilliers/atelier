...
function loadSessions(projectId: string): { sessions: SessionSummary[]; error: string | null; dbPath: string } {
  try {
...
export default function RunsPage({ params }: { params: { project: string } }) {
  const now = Date.now();
  const { sessions, error } = loadSessions(params.project);
        ...
        <>
          ...
          {sessions.length === 0 ? (
            ...
            <div className="border border-os-border">
              {sessions.map((s) => (
                <RunRow key={s.adw_id} session={s} now={now} projectId={params.project} />
              ))}
            </div>
          )}
        </>
...
function RunRow({ session, now, projectId }: { session: SessionSummary; now: number; projectId: string }) {
  ...
  return (
    <Link
      href={projectHref(projectId, `/runs/${session.adw_id}`)}
